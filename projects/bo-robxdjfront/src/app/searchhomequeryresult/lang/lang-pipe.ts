...
export function createTranslateLoader(http: HttpClient,version:string) {
  ...
  return new TranslateHttpLoader(http, '/apps/fssp/fssc/web/bo-robxdjfront/searchhomequeryresult/i18n/', '.json'+ versionSuffix);
}

export const lang = {"zh-CHS":{"root-component":"","qdp-framework-component":"报销单据查询","spreadSheet1":""}};
    ...
    if (langCode == "zh-CHS" || (this.translate.defaultLang === langCode && this.translate.currentLoader == createTranslateLoader(this.http,null))) {
      ...
      const httpOb = this.http.get("/apps/fssp/fssc/web/bo-robxdjfront/version.json?v=" + new Date().getTime()).pipe(switchMap((data)=>{
        ...
        if (data instanceof Array) {
          const versionKey = "searchhomequeryresult/" + langCode + ".json";
          data.forEach((item) => {
